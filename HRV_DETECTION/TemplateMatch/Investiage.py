...
def Initpeak(data,style="peak"):
    ...
    while (True):
        ...
        if len(index)==0 : # 当首个检测时
            index.append(beat)
        elif (beat-index[-1])>120 :
            peak = findpeak(hy[win_min:win_min+50]) + win_min
            if  len(peak)==0 :
                index.append(beat)
            else :
                beat = int(np.argmax(Amp_JK[win_min:win_min+50]) + win_min)
                index.append(beat)
        else:
            index.append(beat)
        win_min = max(0, beat + 50)
        win_max = min(length, beat + 150)
        if (win_min >= length-3):
            break
